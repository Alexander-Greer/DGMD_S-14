...
void Accelero_Sensor_Handler( void *handle , uint32_t msTick, uint32_t *msTickStateChange, uint8_t *state )
{
  ...
  if ( status == 1 )
  {
    ...
    if(SendOverUSB) /* Write data on the USB */
    {
    	...
    	/*
    	// convert the int32_t data type into float data type to enable floating point computation
        ...
        // State machine implementaiotn is here
...
void Gyro_Sensor_Handler( void *handle )
{
  ...
  if ( status == 1 )
  {
    ...
    {
    	uint32_t abs_vel;
		abs_vel = ((int)angular_velocity.AXIS_X * (int)angular_velocity.AXIS_X);
		abs_vel += ((int)angular_velocity.AXIS_Y * (int)angular_velocity.AXIS_Y);
    	abs_vel += ((int)angular_velocity.AXIS_Z * (int)angular_velocity.AXIS_Z);
		abs_vel = sqrt((float) abs_vel);
      ...
      // BELOW \/ FROM TUTORIAL 2, COMMENTED FOR CONVENIENCE
      /*
      if (abs_vel > 40000)
            {
          	  sprintf( dataOut, "\n\r\t\t\tVelocity Vector Magnitude > 40,000deg/sec!");
          	  CDC_Fill_Buffer(( uint8_t * )dataOut, strlen( dataOut ) );
            }
      */
